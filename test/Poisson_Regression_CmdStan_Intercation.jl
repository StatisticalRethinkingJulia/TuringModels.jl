...
nsneeze_data = vcat(rand(noalcohol_meds, q), rand(alcohol_meds, q),
  rand(noalcohol_nomeds, q), rand(alcohol_nomeds, q) )
alcohol_data = vcat(zeros(q), ones(q), zeros(q), ones(q) )
meds_data = vcat(zeros(q), zeros(q), ones(q), ones(q) )

inter_alcohol_meds = [meds_data[i]==1 || alcohol_data[i]==1 ? 1 : 0 for i in 1:4q]

df = DataFrame(nsneeze = nsneeze_data, alcohol_taken = alcohol_data,
  nomeds_taken = meds_data, inter_alcohol_meds = inter_alcohol_meds)
first(df, 10), last(df, 10)

# Convert the DataFrame object to matrices.

data = Matrix(df[[:alcohol_taken, :nomeds_taken, :inter_alcohol_meds]])
